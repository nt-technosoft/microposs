...
- [x] T-1.3 Пересобрать остальные секции детальной страницы — все секции переведены на shadcn `Card` + OKLCH + numbers-first, единый спокойный язык. Контракты 1:1, проверено вживую.
  ...
  - [x] T-1.3g Шиты/модалки переведены на новый язык (раньше были на legacy — основатель верно заметил): конвертация валюты пула (полностью переосмыслена — человекопонятные тексты, реальный «отдаёте → получаете», курс из сервиса ЦБ, без фейк-селекта), правка товара (фикс биндинга shadcn `Input` qty; «Удалить строку» → «Удалить позицию»), выбор товара, быстрое создание товара, правка расхода. Спокойные green-tint чипы, terracotta-удаление, shadcn `Input`/`Button`. Проверено вживую. Остаются глубже: `PaymentMakeSheet`, `ReceiveBatchConfirmSheet`, `PartnershipCapitalPaymentSheet` — следующий заход.
