...
### Фаза 5 — visual convergence после founder review

- [x] Убрать двойное ограничение ширины shell/container и открыть desktop canvas.
- [x] Пересобрать sidebar в компактную навигационную рейку с однострочными
  секциями и постоянным положением collapse control.
- [x] Устранить локальные desktop caps и фоновые полосы на business-маршрутах,
  сохранив mobile/tablet presentation.
- [x] Повторно проверить `1440×900`, `1280×800`, `1024×768` и `390×844`.

### Фаза 6 — desktop composition refinement

- [x] Сделать desktop-секции sidebar сворачиваемыми с сохранением состояния и
  обязательным раскрытием активной секции.
- [x] Упростить переключатель валюты отчётов и сохранить provenance только для
  действительно пересчитанного USD-представления.
- [x] Перенести состояние смены в компактную верхнюю desktop-панель, а фильтр
  категории — рядом с поиском товаров.
- [x] Развязать высоту form/context колонок в перемещениях, обмене валют,
  инвесторах и карточке инвестдоговора.
- [x] Перевести настройки и карту разделов на независимый flow без пустых
  grid-прослоек.
- [x] Повторить typecheck, lint, build, focused Vitest, browser-smoke и свежий
...
- Ограничение baseline: на двух активных бизнесах старый E21 auth-контракт не
  ...
  итерацией.
- После визуального founder review E31 повторно открыт и доведён до единой
  desktop-композиции: sidebar сокращён до `232/64px`, заголовки секций отделены
  от legacy `h2`-каскада, collapse control закреплён у верхнего края, а общий
  shell и list/workbench-страницы больше не формируют центрированный
  `1200/1120px` остров внутри доступного canvas. Ограничения сохранены только у
  форм и читаемых документов. Mobile-контракт и бизнес-логика не менялись.
- Фаза 6 закрыла оставшиеся системные stretch-дефекты: общие desktop-примитивы
  теперь поддерживают независимые sticky/flow-колонки, а прикладные страницы не
  наследуют высоту более длинного соседа. Каталог, продажи, отчёты и sidebar
  уплотнены без изменения их данных, событий, маршрутов или permissions.
...
- **2026-07-20:** технически завершённый shell повторно открыт после founder
  review: наличие sidebar не считается достаточной desktop-адаптацией, если
  навигационная иерархия сломана каскадом, toggle меняет положение, а страницы
  остаются узкими островами. Новое правило E31 — один canvas и task-adaptive
  внутренняя композиция без изменения маршрутов и доменного поведения.
- **2026-07-22:** для неравных desktop-колонок принят natural-height contract:
  рабочая и контекстная поверхности самостоятельно определяют высоту, sticky
  используется только там, где сохраняет текущую задачу в поле зрения, а
  masonry-like flow допустим для независимых информационных карточек. Это
  presentation-правило E31 и не меняет мобильный DOM-порядок или бизнес-логику.
