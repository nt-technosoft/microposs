...
import { createInvestmentFund, fetchInvestmentFunds, type InvestmentFund } from '@/api/partnerships'
...
function toggleMember(id: number): void {
  memberIds.value = memberIds.value.includes(id)
    ? memberIds.value.filter((value) => value !== id)
    : [...memberIds.value, id]
}
...
async function load(): Promise<void> {
  ...
  error.value = ''
  try {
    const [fundRows, partnerRows] = await Promise.all([
...
async function submit(): Promise<void> {
  if (!name.value.trim() || !managerId.value || !memberIds.value.length) {
    error.value = 'Укажите название, управляющего и участников фонда.'
    return
  }
  saving.value = true
  error.value = ''
  try {
    const fund = await createInvestmentFund({
      ...
      member_partner_ids: memberIds.value,
    ...
    toast.success('Фонд создан: до первого размещения можно собрать капитал.')
...
<template>
  <main class="min-h-dvh bg-background pb-[calc(var(--bottom-nav-height)+1rem)]">
    ...
    <section class="mx-auto flex w-full max-w-[var(--max-content-width)] flex-col gap-4 px-4 py-4 sm:px-6">
      ...
      <div v-else-if="!funds.length" class="flex flex-col items-center gap-2 border border-dashed border-border py-14 text-center"><Landmark class="size-8 text-muted-foreground" /><strong>Фондов пока нет</strong><span class="max-w-sm text-sm text-muted-foreground">Создайте закрытый фонд, согласуйте условия офлайн и соберите капитал до первого размещения.</span></div>
      <div v-else class="divide-y divide-border border-y border-border">
        <button v-for="fund in funds" :key="fund.id" class="flex w-full items-center justify-between gap-4 py-4 text-left transition hover:bg-muted/40" type="button" @click="router.push({ name: 'fund-detail', params: { id: fund.id } })"><span class="min-w-0"><span class="flex items-center gap-2 font-medium text-foreground"><Users class="size-4 text-primary" />{{ fund.name }}</span><span class="mt-1 block text-sm text-muted-foreground">{{ formatStatus(fund.status) }} · {{ fund.members.length }} участников · управляющий {{ fund.manager_partner_name }}</span></span><strong class="shrink-0 tabular-nums text-foreground">{{ formatPrice(fund.positions.reduce((sum, row) => sum + Number(row.paid_in || 0), 0), fund.currency) }}</strong></button>
      </div>
    </section>
  </main>
</template>
