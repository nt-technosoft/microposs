  ...
  <main class="min-h-dvh bg-background pb-[calc(var(--bottom-nav-height)+1rem)]">
    <PageContainer size="wide" :padded="false" class="py-4 sm:py-6">
      <PageChrome
        ...
        description="Инвестиционные условия, свободный капитал и связанные приходы."
      >
        <template #primary>
      ...
      <div class="px-4 sm:px-6 lg:px-8">
        <ListWorkbench aside="narrow" sticky-aside>
          <template #aside>
            <dl class="grid grid-cols-2 divide-x divide-border overflow-hidden rounded-[14px] border border-border bg-surface lg:grid-cols-1 lg:divide-x-0 lg:divide-y">
              <div class="p-4">
                <dt class="text-xs text-muted-foreground">{{ t('procurements.activeAgreements') }}</dt>
                <dd class="mt-1 text-xl font-semibold tabular-nums text-foreground">{{ totals.open }}</dd>
              </div>
              <div class="min-w-0 p-4">
                <dt class="text-xs text-muted-foreground">{{ t('procurements.plannedBudget') }}</dt>
                <dd class="mt-1 break-words text-lg font-semibold tabular-nums text-foreground">{{ plannedBudgetLabel }}</dd>
              </div>
            </dl>
          </template>
          ...
          <div v-else class="divide-y divide-border overflow-hidden rounded-[14px] border border-border bg-surface">
            <button
              v-for="agreement in agreements"
              :key="agreement.id"
              class="flex w-full flex-col items-stretch gap-2 px-4 py-3.5 text-left transition hover:bg-muted/50 sm:flex-row sm:items-center sm:justify-between sm:gap-4"
              type="button"
